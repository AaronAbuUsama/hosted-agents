...
const INITIAL_ITEMS: WorkItem[] = [
  ...
  },
  {
    ...
    description: "Create a first-pass layout for the landing page and share it for early feedback.",
    lastEdited: "18m ago",
    dueDate: "Jul 3",
  },
  {
...
const styles = {
  ...
  floatingAt: (x: number, y: number, width: number): CSSProperties => ({
    width,
    transform: `translate(${x}px, ${y}px)`,
  }),
  // Placeholder marking the landing slot; matches the dragged card's height.
  ghost: (height: number): CSSProperties => ({
    height,
          ...
          <LayoutContent ref={contentRef} padding={2}>
            {children ?? (
              <EmptyState
                ...
                icon={<Icon icon={meta.emptyIcon} size="lg" color="secondary" />}
...
export default function KanbanBoardTemplate() {
  ...
  const moveItem = (id: string, to: ColumnId) => {
    setItems((prev) => prev.map((item) => (item.id === id ? { ...item, column: to } : item)));
  };
  ...
  const renderColumnCards = (colId: ColumnId): ReactNode => {
    ...
    if (ghostTarget && ghostTarget.target) {
      ...
      nodes.splice(index, 0, <VStack key="drag-ghost" style={styles.ghost(ghostTarget.height)} />);
    }
              ...
              endContent={
                <HStack gap={2}>
                  ...
                  <HStack gap={1} vAlign="center">
                    <IconButton icon={<Icon icon={ArrowsUpDownIcon} size="sm" />} label="Sort" />
                    <IconButton icon={<Icon icon={FunnelIcon} size="sm" />} label="Filter" />
                    <IconButton
