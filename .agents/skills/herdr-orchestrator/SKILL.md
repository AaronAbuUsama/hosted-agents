...
- **Readiness ≠ `agent-status idle` for interactive TUI agents.** Verified: herdr false-detects the agent from the typed command word, so `idle` reports *before* the agent can accept input. Wait for the real UI marker: `herdr wait output "$P" --source visible --match "shortcuts"` (Claude Code), then send the task.
...
- **`wait output` matches the echoed command too** — `pane run` puts the command in the buffer, so pick a match string that only appears in real *output* (not in what you sent).
